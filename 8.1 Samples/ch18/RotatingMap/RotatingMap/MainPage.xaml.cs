...
namespace RotatingMap
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        void GetLongitudeAndLatitude(Geocoordinate geoCoordinate)
        {
            ...
            double relativeLongitude = (180 + geoCoordinate.Point.Position.Longitude) / 360;
            integerLongitude = (int)(relativeLongitude * (1 << BITRES));

            //double sinTerm = Math.Sin(Math.PI * geoCoordinate.Latitude / 180);
            double sinTerm = Math.Sin(Math.PI * geoCoordinate.Point.Position.Latitude / 180);
        ...
        AppBarButton GetZoomAppBarButton(string name)
        {
            
            var btn = bottomAppBar.PrimaryCommands.Where(b => ((FrameworkElement)b).Name == name).FirstOrDefault() as AppBarButton;
            return btn;
        }
        AppBarToggleButton GetStreetViewAppBarButton(string name)
        {

            var btn = bottomAppBar.PrimaryCommands.Where(b => ((FrameworkElement)b).Name == name).FirstOrDefault() as AppBarToggleButton;
            return btn;
        }
